...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, ShieldAlert, Fingerprint, Lock, Eye } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Security IQ</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/attacks" icon={<ShieldAlert size={20} />} label="Attack Insights" />
            <NavItem to="/waf" icon={<Lock size={20} />} label="WAF Rules" />
            <NavItem to="/bots" icon={<Fingerprint size={20} />} label="Bot Management" />
            <NavItem to="/incidents" icon={<AlertTriangle size={20} />} label="Incident Center" />
            <NavItem to="/observability" icon={<Eye size={20} />} label="Edge Telemetry" />
            <NavItem to="/compliance" icon={<BarChart4 size={20} />} label="Posture Scorecards" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              ...
              <span>/</span>
              <span className="text-white font-bold">Global Application Protection</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Security Architect</p>
                <p className="text-[10px] text-rose-400 uppercase tracking-widest font-black">Industrialized Defense</p>
              </div>
